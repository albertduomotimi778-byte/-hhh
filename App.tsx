...
${isNodeProject ? `
      ...
      - name: Build
        env:
           CI: false 
        # Run build with relative base path to support renaming/deployment to any subpath
...
const processZipFile = async (file: File): Promise<ProcessedZip> => {
  ...
  const hasPackageJson = filePaths.some(p => p === rootPrefix + 'package.json');
  ...
  if (!hasPackageJson && !files['index.html']) {
     throw new Error("Could not find 'index.html' or 'package.json' in the zip file.");
  }

  return { files, isNodeProject: hasPackageJson, hasAiDependency };
};
...
interface SelectionStepProps {
  user: GitHubUser;
  onDeploy: (repoName: string, isNew: boolean, files: Record<string, string> | null, isNodeProject: boolean, apiKey: string | null, templateId?: string) => void;
...
const SelectionStep = ({ user, onDeploy, initialRepoName }: SelectionStepProps) => {
  ...
  const [zipAnalysis, setZipAnalysis] = useState<{isNode: boolean, count: number, hasAiDependency: boolean} | null>(null);
  ...
  return (
    <div className="max-w-4xl mx-auto w-full animate-in">
      <div className="bg-white rounded-2xl shadow-xl border border-slate-100 overflow-hidden flex flex-col md:flex-row min-h-[500px]">
        ...
        <div className="flex-1 p-8 flex flex-col">
          <h2 className="text-xl font-bold text-slate-900 mb-6">
            {activeTab === 'upload' ? 'Deploy from Zip' : 'Deploy from Template'}
          </h2>
                     ...
                     <div className="space-y-3">
                       <div className="text-sm bg-green-50 text-green-700 p-3 rounded-lg border border-green-100 flex items-start">
                          ...
                          <div>
                              ...
                              {zipAnalysis.isNode ? (
                                  <p className="text-xs mt-1 font-medium">Detected Node.js project.</p>
                              ) : (
                                  <p className="text-xs mt-1 font-medium">Detected static site.</p>
                              )}
                          </div>
                       </div>
          ...
          <div className="pt-6 mt-6 border-t border-slate-100 flex justify-end">
            ...
            >
              {activeTab === 'upload' ? 'Upload & Deploy' : 'Create & Deploy'}
...
const App = () => {
  ...
  const handleDeploy = async (repoName: string, isNew: boolean, zipFiles: Record<string, string> | null, isNodeProject: boolean, apiKey: string | null, templateId?: string) => {
    if (!user) return;
    ...
    try {
      ...
      // IMPORTANT: We now enforce 'workflow' build type for ALL projects (static or node)
      // This is because the legacy Jekyll/classic build often results in 404s for SPA or unconfigured roots,
      // and gives no logs. Actions are more reliable.
...
`.trim();
        ...
        if (isNodeProject && zipFiles['package.json']) {
           try {
      ...
      const workflowContent = getWorkflowYaml(isNodeProject);
